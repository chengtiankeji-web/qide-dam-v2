...
COUNT=$(echo "$RESP" | python3 -c "import sys,json; print(json.load(sys.stdin).get('total',-1))")
...
echo ""
echo "▶ Cleanup"
curl -s -X DELETE "$API/v1/vault/$VAULT_ID" -H "Authorization: Bearer $TOKEN" -o /dev/null
curl -s -X DELETE "$API/v1/auth/api-keys/$KEY_ID" -H "Authorization: Bearer $TOKEN" -o /dev/null
curl -s -X DELETE "$API/v1/auth/api-keys/$KEY_VAULT_ID" -H "Authorization: Bearer $TOKEN" -o /dev/null
echo "  cleaned"
